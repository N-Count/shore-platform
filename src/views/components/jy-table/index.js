...
import JySelect from './fields/select'
...
export default {
  ...
  methods: {
    ...
    // 渲染表格
    $_renderTable (h) {
      const { data, tableHeight, innerHeight, rowKey, showHover, tableContainerHeight, heigth, maxHeight, emptyText } = this
      ...
      /**
       * 备注
       *  这里之前使用的 ...this.$attrs 发现外部添加的ElementUI官方的属性不起作用 所以改成了 ...this.$props
       */
      const props = {
        ...originTableProps,
        ...this.$props
      }
      if (heigth && heigth !== 'auto') {
        props['height'] = tableHeight
      }
      if (maxHeight) {
        props['max-height'] = maxHeight
      }
      const table = (
        <Table
          data={data}
          ...
          {...{
            on: {
              ...
              'cell-mouse-enter':this.$_handleCellMouseEnter,
              'cell-mouse-leave':this.$_handleCellMouseLeave,
    ...
    $_getSelectionColumn (h) {
      ...
      return selectable
        ? [<TableColumn type="selection" width="" fixed="left" />]
        : []
    },
    // 获取序号列
          ...
          {...{
            ...
            scopedSlots: {
              default: ({ row, column, $index }) => {
                ...
                return (
                  <Link type="primary" onClick={onClick} class="jy-table__link">
                    {row[prop]}
                  </Link>
                )
              }
    ...
    $_renderDefaultColumn (h, column) {
      const { events = {}, minWidth = '', ...rest } = column
      return (
        <TableColumn
          minWidth={minWidth}
          showOverflowTooltip
    ...
    $_renderActionColumn (h, column) {
      ...
      return (
        <TableColumn
          ...
          align={align}
          width={width}
          className="jy-table__actioncolumn"
          {...{
            scopedSlots: {
              default: ({ row, column, $index }) => {
    ...
    $_renderEditColumn (h, column) {
      ...
      return (
        <TableColumn
          ...
          align={align}
          showOverflowTooltip
          labelClassName={isRequired ? 'jy-table__column--required' : ''}
          {...{
            scopedSlots: {
              default: scope => {
                if (
    ...
    $_renderToolbar (h) {
      ...
      if (buttons || toolbarSlot) {
        return (
          <div class="jy-table__toolbar">
            <div class="jy-table__buttons">
              {buttons}
              {/** 可以将自定义内容插入工具条 */}
              {toolbarSlot}
            </div>
    ...
    // 渲染分页
    $_renderPage (h) {
      const { pageSize, total, currentPage, fixed = true } = this
      if (total < 10) return
      // pageSizes={[5, 10, 20, 50]}
      return (
        <div class={`${fixed ? 'jy-flex-center-row' : 'jy-default-pagination-footer'} jy-table__page`}>
          <Pagination
            total={total}
    ...
    $_handleRowClick (row, column, event) {
      // 行点击时 是否只能选中一行
      if (this.isSelectNowRow) {
        this.$refs.table.clearSelection()
        this.toggleRowSelection(row, true)
      }
      this.$emit('row-click', row, column, event)
    },
    $_handleCellMouseEnter (row, column, event) {
      this.$emit('cellHoverIn', row, column, event)
    },
    $_handleCellMouseLeave (row, column, event) {
      this.$emit('cellHoverOut', row, column, event)
    },
    // 表格的当前行发生变化的时候会触发该事件
  ...
  render (h) {
    ...
    const footer = this.$_renderPage(h)
    const { isPagination } = this
    return (
      <div>
        {toolbar}
        {table}
