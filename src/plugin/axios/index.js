...
function errorCreate(msg, response) {
  ...
  if (window.CONFIG?.CLOUD_VUE_APP_ENABLE_SENTRY && response?.config?.baseURL) {
    ...
    const warningWord = window.CONFIG.CLOUD_SENTRY_KEYWORDS || [
      'error',
      '异常',
      '系统',
      'not found'
    ...
    if (
      warningWord.find(i => msg?.toLowerCase().includes(i)) ||
      response?.data?.code.includes('9999')
    ) {
      console.error(
...
const service = axios.create({
  baseURL: process.env.NODE_ENV === 'production' ? window.CONFIG.CLOUD_VUE_APP_API : config.baseURL,
...
service.interceptors.request.use(
  config => {
    ...
    // 设备类型
    config.headers['deviceType'] = /Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)
      ? 'PHONE'
      : 'PC'
    // 网络类型
